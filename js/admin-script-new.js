...
const fixAdminImagePath = (imagePath) => {
    ...
    // Pages is in pages/, so we need ../ to get to root
    return '../' + imagePath;
};
...
async function viewCustomer(customerId) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api_customers.php?action=get&id=${customerId}`);
...
async function saveCustomerChanges(event) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api_customers.php?action=update&id=${customerId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
