...
// API Base URL - dynamically detect base path and add /api
const getApiBaseUrl = () => {
    const path = window.location.pathname;
    // Find the base path (e.g., /snoptical or /snoptical/pages)
    const pathParts = path.split('/').filter(p => p);
    // The first part should be the project folder
    const projectFolder = pathParts[0] || '';
    return window.location.origin + '/' + projectFolder + '/api';
};
const API_BASE_URL = getApiBaseUrl();

// CSRF Token Helper - Get token from meta tag
const getCsrfToken = () => {
    const meta = document.querySelector('meta[name="csrf-token"]');
    return meta ? meta.getAttribute('content') : '';
};

// Create fetch wrapper that includes CSRF token
...
// Helper to get root base URL (for image paths from pages folder)
const getScriptRootBaseUrl = () => {
    const path = window.location.pathname;
    const pathParts = path.split('/').filter(p => p);
...
// Helper to fix image paths (add ../ prefix when in pages folder for relative paths)
const fixScriptImagePath = (imagePath) => {
    if (!imagePath) return 'https://images.unsplash.com/photo-1574258495973-f010dfbb5371?w=400&h=400&fit=crop';
    // If it's already an absolute URL, return as-is
    if (imagePath.startsWith('http://') || imagePath.startsWith('https://') || imagePath.startsWith('data:')) {
        return imagePath;
    }
    // If it's a relative path (assets/, uploads/), prefix with root base URL
    const rootBase = getScriptRootBaseUrl();
...
function transformProduct(dbProduct) {
    ...
    return {
        ...
        images: dbProduct.image ? dbProduct.image.split(',').map(img => fixScriptImagePath(img.trim())).filter(img => img) : [],
...
function getFirstImage(imageStr) {
    ...
    return images.length > 0 ? fixScriptImagePath(images[0]) : null;
}
...
async function fetchProductsFromAPI() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api_products.php?action=list&status=active`);
...
function createProductCard(product) {
    ...
    const pagesBase = getScriptPagesBaseUrl();
    card.innerHTML = `
        <a href="${pagesBase}product-detail.php?id=${productData.id}" class="product-link">
...
function displayCartItems() {
    ...
    cart.forEach(item => {
        ...
        // Get first image from comma-separated list for products with multiple images (with path fixing)
        const displayImage = getFirstImage(item.image) || fixScriptImagePath(item.image) || 'https://images.unsplash.com/photo-1574258495973-f010dfbb5371?w=400&h=400&fit=crop';
...
function proceedToCheckout() {
    ...
    window.location.href = getScriptPagesBaseUrl() + 'checkout.php';
...
function displayWishlistItems() {
    ...
    let itemsHTML = '';
    const pagesBaseForWishlist = getScriptPagesBaseUrl();
    wishlist.forEach(product => {
        ...
        itemsHTML += `
            <div class="wishlist-item" style="display: flex; gap: 15px; padding: 15px; border-bottom: 1px solid #e0e0e0; background: #f9f9f9; border-radius: 8px; margin-bottom: 15px;">
                <a href="${pagesBaseForWishlist}product-detail.php?id=${product.id}" style="flex-shrink: 0;">
                    <img src="${productImage}" alt="${product.name}" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px;">
                </a>
                <div style="flex: 1;">
                    <a href="${pagesBaseForWishlist}product-detail.php?id=${product.id}" style="text-decoration: none;">
