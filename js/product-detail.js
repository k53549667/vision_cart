...
const fixImagePath = (imagePath) => {
    ...
    const rootBase = getProductRootBaseUrl();
...
async function loadCart() {
    try {
        const response = await fetch(`${PRODUCT_API_BASE_URL}/api_cart.php?action=list`, {
            credentials: 'include'
        });
...
async function addToCart(productId) {
    try {
        ...
        let response = await fetch(`${PRODUCT_API_BASE_URL}/api_cart.php?action=add`, {
            method: 'POST',
            credentials: 'include',
        ...
        if (!data.success && data.error === 'Product not found') {
            ...
            if (localProduct) {
                ...
                const createResponse = await fetch(`${PRODUCT_API_BASE_URL}/api_products.php?action=create`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                if (createResult.success && createResult.id) {
                    ...
                    response = await fetch(`${PRODUCT_API_BASE_URL}/api_cart.php?action=add`, {
                        method: 'POST',
                        credentials: 'include',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
...
function proceedToCheckout() {
    ...
    if (!savedUser) {
        ...
        window.location.href = getProductRootBaseUrl() + 'pages/login.php';
        return;
    }
...
function getImagesArray(imageStr) {
    if (!imageStr) return [];
    return imageStr.split(',').map(img => fixImagePath(img.trim())).filter(img => img);
}
...
async function findProductById(id) {
    try {
        ...
        const response = await fetch(`${PRODUCT_API_BASE_URL}/api_products.php?action=get&id=${id}`);
        if (response.ok) {
            const product = await response.json();
            if (product && !product.error) {
                console.log('✅ Found product in database:', product);
                // Parse images from comma-separated string (with path fixing)
...
async function loadProductDetails() {
    ...
    if (!productId) {
        window.location.href = getProductRootBaseUrl() + 'index.php';
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check wishlist status for current product
    const productId = getProductIdFromURL();
    if (productId) {
...
async function addToWishlist(productId) {
    ...
    try {
        ...
        const response = await fetch(`${PRODUCT_API_BASE_URL}/api_wishlist.php?action=toggle`, {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success) {
            if (data.action === 'added') {
                showNotification(`${data.product_name || 'Product'} added to wishlist!`, 'success');
                ...
                if (wishlistBtn) {
                    ...
                    wishlistBtn.style.borderColor = '#ff6b6b';
                }
            } else {
                showNotification(`${data.product_name || 'Product'} removed from wishlist`, 'info');
                // Reset button
                if (wishlistBtn) {
        ...
        if (wishlistBtn) {
            ...
            // Restore button text if it shows loading
...
async function checkWishlistStatus(productId) {
    try {
        const response = await fetch(`${PRODUCT_API_BASE_URL}/api_wishlist.php?action=check&product_id=${productId}`, {
            credentials: 'include'
        });
        const data = await response.json();
