                    ...
                    <div class="round-product-grid" id="productGrid">
                        <?php if ($productCount > 0): ?>
                            <?php foreach ($products as $product): 
                                ...
                                // Fix image path - get first image and add ../ prefix for relative paths
                                $rawImage = !empty($product['image']) ? explode(',', $product['image'])[0] : '';
                                $rawImage = trim($rawImage);
                                if (!empty($rawImage) && !preg_match('/^https?:\/\//', $rawImage)) {
                                    $imageUrl = '../' . $rawImage;
