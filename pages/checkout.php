...
<head>
    ...
    <script>
        // Server-side auth already checked, but also sync with client state
        const serverUser = {
            id: <?php echo json_encode($userId); ?>,
            name: <?php echo json_encode($userName); ?>,
            email: <?php echo json_encode($userEmail); ?>
        };
        // Store in localStorage to keep client in sync
        if (serverUser.id) {
            localStorage.setItem('visionkart_user', JSON.stringify(serverUser));
        }
    </script>
    <style>
        * {
...
<body>
    ...
    <script>
        ...
        async function checkUserAuth() {
            try {
                const response = await fetch('../api/api_auth.php?action=check', {
                    credentials: 'include'
                });
        ...
        async function loadCheckoutData() {
            ...
            try {
                // Fetch cart from API
                const response = await fetch('../api/api_cart.php?action=list', {
                    credentials: 'include'
                });
                const data = await response.json();
        ...
        document.getElementById('placeOrderBtn').addEventListener('click', async function() {
            // Double-check login status before placing order
            ...
            try {
                // Send order to API
                const response = await fetch('../api/api_orders.php?action=create', {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    ...
                    await fetch('../api/api_cart.php?action=clear', {
                        method: 'DELETE',
                        credentials: 'include'
                    });
