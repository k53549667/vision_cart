...
/**
 * User Dashboard
 * Requires user authentication (server-side check)
 */
session_start();

// Server-side authentication check (Critical Security Fix)
if (!isset($_SESSION['user_id'])) {
    // Store intended destination for post-login redirect
    $_SESSION['redirect_after_login'] = 'dashboard.php';
    header('Location: login.php?message=Please+login+to+access+your+dashboard');
    exit;
}
...
<head>
    ...
    <style>
        ...
        .wishlist-card:hover {
            border-color: #00bac7;
            box-shadow: 0 5px 20px rgba(0, 186, 199, 0.15);
...
<body>
        ...
        <div class="dashboard-grid">
            ...
            <aside class="dashboard-sidebar">
                <ul class="sidebar-menu">
                    ...
                    <li><a href="#" class="menu-item" data-section="wishlist">
            ...
            <main class="dashboard-content">
                ...
                <section id="overview" class="section active">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card" style="cursor: pointer;" onclick="document.querySelector('[data-section=orders]').click()">
                            <i class="fas fa-clock"></i>
                            <div class="stat-value" id="pendingOrders">0</div>
                            <div class="stat-label">Pending Orders</div>
                        </div>
                        <div class="stat-card" style="cursor: pointer;" onclick="document.querySelector('[data-section=wishlist]').click()">
                ...
                <section id="wishlist" class="section">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                        <p style="margin: 0; color: #666;">Products you've saved for later</p>
                        <button class="btn btn-danger btn-small" onclick="clearAllWishlist()" id="clearWishlistBtn" style="display: none;">
                            <i class="fas fa-trash-alt"></i> Clear All
                        </button>
                    </div>

                    <div id="wishlistList" class="addresses-grid">
                        <div class="loading">
                            <i class="fas fa-spinner"></i>
    ...
    <script>
        ...
        async function loadUserWishlist() {
            try {
                const response = await fetch('../api/api_wishlist.php?action=list', {
                    credentials: 'include'
                });
                ...
                if (data.success && data.items && data.items.length > 0) {
                    ...
                    wishlistList.innerHTML = data.items.map(item => `
                        <div class="wishlist-card">
                            <a href="product-detail.php?id=${item.id}">
                                <img src="${item.image || 'https://images.unsplash.com/photo-1574258495973-f010dfbb5371?w=400&h=400&fit=crop'}" 
                                     alt="${item.name}" class="wishlist-card-image">
                            </a>
                            <div class="wishlist-card-content">
                                <a href="product-detail.php?id=${item.id}" style="text-decoration: none;">
                                    <div class="wishlist-card-name">${item.name}</div>
                                </a>
                                ${item.brand ? `<div class="wishlist-card-brand">${item.brand}</div>` : ''}
                                <div class="wishlist-card-price">
                                    <span class="current">₹${item.price.toLocaleString()}</span>
                                    ${item.original_price && item.original_price > item.price ? 
                                        `<span class="original">₹${item.original_price.toLocaleString()}</span>
                                         <span class="discount">${item.discount}% OFF</span>` : ''}
                                </div>
                                <div class="wishlist-card-stock ${item.in_stock ? 'in-stock' : 'out-of-stock'}">
                                    <i class="fas ${item.in_stock ? 'fa-check-circle' : 'fa-times-circle'}"></i>
                                    ${item.in_stock ? 'In Stock' : 'Out of Stock'}
                                </div>
                                <div class="wishlist-card-actions">
                                    <button class="btn btn-primary btn-small" onclick="addWishlistToCart(${item.id})" ${!item.in_stock ? 'disabled' : ''}>
                                        <i class="fas fa-shopping-cart"></i> Add to Cart
                                    </button>
                                    <button class="btn btn-danger btn-small" onclick="removeWishlistItem(${item.id})">
        ...
        async function addWishlistToCart(productId) {
            try {
                const response = await fetch('../api/api_cart.php?action=add', {
                    method: 'POST',
                ...
                if (data.success) {
                    showDashboardAlert('wishlistAlert', 'success', `${data.product_name || 'Product'} added to cart!`);
                    // Optionally remove from wishlist after adding to cart
                    // await removeWishlistItem(productId);
                } else {
                    showDashboardAlert('wishlistAlert', 'error', data.error || 'Failed to add to cart');
                }
            } catch (error) {
                console.error('Error adding to cart:', error);
                showDashboardAlert('wishlistAlert', 'error', 'Failed to add to cart. Please try again.');
            }
        }
        ...
        async function removeWishlistItem(productId) {
            try {
                const response = await fetch(`../api/api_wishlist.php?action=remove&product_id=${productId}`, {
                    method: 'DELETE',
                    credentials: 'include'
                });
                const data = await response.json();
                ...
                if (data.success) {
                    showDashboardAlert('wishlistAlert', 'success', 'Item removed from wishlist');
                    await loadUserWishlist();
                    await loadUserStats();
                } else {
                    showDashboardAlert('wishlistAlert', 'error', data.error || 'Failed to remove item');
                }
            } catch (error) {
                console.error('Error removing from wishlist:', error);
                showDashboardAlert('wishlistAlert', 'error', 'Failed to remove item. Please try again.');
            }
        }
        ...
        async function clearAllWishlist() {
            ...
            try {
                const response = await fetch('../api/api_wishlist.php?action=clear', {
                    method: 'DELETE',
                    credentials: 'include'
                });
                const data = await response.json();
                ...
                if (data.success) {
                    showDashboardAlert('wishlistAlert', 'success', 'Wishlist cleared successfully');
                    await loadUserWishlist();
                    await loadUserStats();
                } else {
                    showDashboardAlert('wishlistAlert', 'error', data.error || 'Failed to clear wishlist');
                }
            } catch (error) {
                console.error('Error clearing wishlist:', error);
                showDashboardAlert('wishlistAlert', 'error', 'Failed to clear wishlist. Please try again.');
            }
        }

        // Show dashboard alert message
        function showDashboardAlert(elementId, type, message) {
        ...
        async function loadUserProfile() {
            try {
                const response = await fetch('../api/api_users.php?action=profile', {
                    credentials: 'include'
                });
                const data = await response.json();
        ...
        async function loadUserStats() {
            try {
                const response = await fetch('../api/api_users.php?action=stats', {
                    credentials: 'include'
                });
                const data = await response.json();
        ...
        async function loadUserOrders() {
            try {
                ...
                const response = await fetch('../api/api_users.php?action=orders', {
                    credentials: 'include'
                });
                const data = await response.json();
        ...
        async function loadUserAddresses() {
            try {
                const response = await fetch('../api/api_users.php?action=addresses', {
                    credentials: 'include'
                });
                const data = await response.json();
        ...
        document.getElementById('profileForm').addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('../api/api_users.php?action=update-profile', {
                    method: 'POST',
                    headers: {
        ...
        document.getElementById('passwordForm').addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('../api/api_auth.php?action=change-password', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        async function loadAddressData(addressId) {
            try {
                const response = await fetch(`../api/api_users.php?action=address&id=${addressId}`, {
                    credentials: 'include'
                });
                const data = await response.json();
        ...
        document.getElementById('addressForm').addEventListener('submit', async (e) => {
            ...
            try {
                const url = addressId 
                    ? `../api/api_users.php?action=update-address&id=${addressId}`
                    : '../api/api_users.php?action=add-address';
        ...
        async function setDefaultAddress(addressId) {
            ...
            try {
                const response = await fetch(`../api/api_users.php?action=set-default-address&id=${addressId}`, {
                    method: 'POST',
                    credentials: 'include'
                });
