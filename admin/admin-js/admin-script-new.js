...
// API Base URL helper - returns main API folder for shared endpoints
...
// Admin API Base URL helper - returns admin-api folder for admin-specific endpoints
const getAdminApiBaseUrl = () => {
    const path = window.location.pathname;
    const pathParts = path.split('/').filter(p => p);
    const projectFolder = pathParts[0] || '';
    return window.location.origin + '/' + projectFolder + '/admin/admin-api';
};

const API_BASE = getMainApiBaseUrl();
const ADMIN_API_BASE = getAdminApiBaseUrl();
...
// Helper to fix image paths for admin panel (add ../../ prefix for relative paths)
const fixAdminImagePath = (imagePath) => {
    if (!imagePath) return 'https://placehold.co/50x50/e0e0e0/666?text=No+Image';
    // If it's already an absolute URL, return as-is
    if (imagePath.startsWith('http://') || imagePath.startsWith('https://') || imagePath.startsWith('data:')) {
        return imagePath;
    }
    // Admin is in admin/admin-pages/, so we need ../../ to get to root
    return '../../' + imagePath;
};
...
function getFirstImage(imageStr) {
    ...
    return images.length > 0 ? fixAdminImagePath(images[0]) : null;
}

// Utility function for API calls (shared APIs)
...
// Utility function for Admin API calls (admin-specific APIs: customers, purchases, users)
...
async function loadCustomers() {
    try {
        cachedCustomers = await adminApiCall('api_customers.php?action=list');
...
async function loadPurchases() {
    try {
        cachedPurchases = await adminApiCall('api_purchases.php?action=list');
...
async function loadProductsTable() {
    ...
    try {
        ...
        products.forEach(product => {
            ...
            row.innerHTML = `
                <td>
                    <img src="${getFirstImage(product.image) || 'https://placehold.co/50x50/e0e0e0/666?text=No+Image'}" alt="${product.name}" style="width: 50px; height: 50px; border-radius: 6px; object-fit: cover;" onerror="this.src='https://placehold.co/50x50/e0e0e0/666?text=No+Image'">
                </td>
                <td>
...
async function capturePhoto() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE}/api_upload.php`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function updateImagePreview() {
    ...
    capturedImages.forEach((imageData, index) => {
        ...
        img.src = fixAdminImagePath(imageData);
        img.onerror = function() {
            this.src = 'https://placehold.co/150x150/e0e0e0/666?text=Error';
        };
        img.style.cssText = `
            width: 100%;
            height: 100%;
...
function openPurchaseModal() {
    ...
    const fields = [
        'purchaseSupplier', 'purchaseSupplierPhone', 'purchaseGstNo', 'purchaseSupplierEmail', 'purchaseCity',
...
async function viewOrder(orderId) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api_orders.php?action=get&id=${orderId}`);
...
async function saveOrderChanges(event) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api_orders.php?action=update&id=${orderId}`, {
            method: 'POST',
...
async function viewCustomer(customerId) {
    ...
    try {
        const response = await fetch(`${ADMIN_API_BASE}/api_customers.php?action=get&id=${customerId}`);
...
async function saveCustomerChanges(event) {
    ...
    try {
        const response = await fetch(`${ADMIN_API_BASE}/api_customers.php?action=update&id=${customerId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
