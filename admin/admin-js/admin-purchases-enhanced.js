...
// API Base URL helper for purchases (admin-specific API folder)
const getPurchasesApiBaseUrl = () => {
    const path = window.location.pathname;
    const pathParts = path.split('/').filter(p => p);
    const projectFolder = pathParts[0] || '';
    return window.location.origin + '/' + projectFolder + '/admin/admin-api';
};
const PURCHASES_API_BASE_URL = getPurchasesApiBaseUrl();
...
async function loadCustomPurchaseCategories() {
    try {
        const response = await fetch(`${PURCHASES_API_BASE_URL}/api_purchase_categories.php?action=all`);
...
async function saveCustomCategory(name, type, parentCategory = null) {
    try {
        const response = await fetch(`${PURCHASES_API_BASE_URL}/api_purchase_categories.php?action=create`, {
            method: 'POST',
...
function savePurchaseFromModal() {
    ...
    const gstNo = document.getElementById('purchaseGstNo')?.value?.trim();
    ...
    // Send to API
    fetch(`${PURCHASES_API_BASE_URL}/api_purchases.php?action=create`, {
        method: 'POST',
        headers: {
...
function loadPurchases() {
    ...
    fetch(`${PURCHASES_API_BASE_URL}/api_purchases.php?action=list`)
        .then(response => response.json())
        .then(purchases => {
