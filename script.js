...
function saveWishlist() {
    localStorage.setItem('visionkart_wishlist', JSON.stringify(wishlist));
    updateWishlistCount();
}
...
function addToWishlist(productId) {
    ...
    // Check localStorage products (admin added)
    if (!product) {
        const storedProducts = JSON.parse(localStorage.getItem('visionkart_products') || '[]');
        product = storedProducts.find(p => p.id === productId);
    }
    
    if (!product) product = roundProducts.find(p => p.id === productId);
    if (!product) product = catEyeProducts.find(p => p.id === productId);
    if (!product) product = clubmasterProducts.find(p => p.id === productId);
    if (!product) product = transparentProducts.find(p => p.id === productId);
...
function setupWishlistSidebar() {
    ...
    if (!wishlistBtn || !wishlistSidebar) return;
...
function displayWishlistItems() {
    ...
    wishlist.forEach(product => {
        itemsHTML += `
            <div style="display: flex; gap: 15px; padding: 15px; border-bottom: 1px solid #e0e0e0; background: #f9f9f9; border-radius: 8px; margin-bottom: 15px;">
                ...
                <div style="flex: 1;">
                    ...
                    <p style="margin: 0 0 10px 0; color: #666; font-size: 12px;">${product.type}</p>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <div><small style="font-size: 10px; color: #666;">Sell Price:</small> <span style="font-size: 16px; font-weight: bold; color: #00bac7;">₹${product.price}</span></div>
                        ${product.originalPrice ? `<div><small style="font-size: 9px; color: #999;">Purchase Price:</small> <span style="text-decoration: line-through; color: #999; font-size: 12px;">₹${product.originalPrice}</span></div>` : ''}
                    </div>
                    <div style="margin-top: 10px; display: flex; gap: 10px;">
                        <button onclick="addToCart(${product.id})" style="flex: 1; padding: 8px; background: #00bac7; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 12px;">
                            <i class="fas fa-shopping-cart"></i> Add to Cart
                        </button>
                        <button onclick="removeFromWishlist(${product.id})" style="padding: 8px 12px; background: #ff6b6b; color: white; border: none; border-radius: 5px; cursor: pointer;">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </div>
